...
interface Trade {
  turn: number; type: "매수" | "매도";
  qty: number; krwPrice: number; snap: Record<string, unknown>; score?: number;
}
interface TurnScore { score: number; maxScore: number; }
interface SaveGameResultParams {
  userId: string; trades: Trade[]; turnScores: TurnScore[];
  totalAsset: number; initCash: number; market: string;
  stockMeta: { ticker: string; name: string };
  interval: string; mission: string | null; followScore: number;
}

export async function saveGameResult({
  userId, trades, turnScores, totalAsset, initCash,
  market, stockMeta, interval, mission, followScore,
}: SaveGameResultParams): Promise<string | null> {
  const supabase = createClient() as any;
  const returnPct = ((totalAsset / initCash) - 1) * 100;
  ...
  if (sessionError || !session) {
    console.error("[saveGameResult] 저장 실패:", sessionError);
    return null;
  }
    ...
    const { error: tradeError } = await supabase
      ...
      .insert(
        trades.map((t, i) => ({
          ...
          turn: t.turn, type: t.type, qty: t.qty,
